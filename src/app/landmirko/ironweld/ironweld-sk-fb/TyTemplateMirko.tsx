    ...
    <div className="ty-mirko">
      <div className="ty-card">
        ...
        <div className="ty-header">
          ...
          <h1>{ty?.title || ''}</h1>
          <p className="ty-subtitle">{ty?.subtitle || ''}</p>
          <span className="ty-order-number">{ty?.orderNumberLabel || ''} {orderNumber}</span>
        </div>

        {/* Riepilogo Ordine */}
        <div className="ty-section">
          <h3><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: '6px' }}><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /><polyline points="3.27 6.96 12 12.01 20.73 6.96" /><line x1="12" y1="22.08" x2="12" y2="12" /></svg>{ty?.summaryTitle || ''}</h3>
          <div className="ty-product-row">
            ...
            <div className="ty-product-info">
              <strong>{priceBox?.productName || ''}</strong>
              <span>{ty?.accessoriesLabel || ''}</span>
            </div>
          ...
          <div className="ty-total-row">
            <span>{ty?.totalLabel || ''}</span>
            <span>{currencySymbol}{totalPrice}</span>
          </div>
          <div className="ty-payment-note">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: '6px' }}><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>{ty?.paymentNote || ''}
          </div>
        </div>
        ...
        {(orderData.name || orderData.phone || orderData.address) && (
          <div className="ty-section">
            <h3><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: '6px' }}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>{ty?.yourDataTitle || ''}</h3>
            <div className="ty-data-grid">
              {orderData.name && (
                <div className="ty-data-row">
                  <span className="ty-data-label">{ty?.nameLabel || ''}</span>
                  <span className="ty-data-value">{orderData.name}</span>
                </div>
              )}
              {orderData.phone && (
                <div className="ty-data-row">
                  <span className="ty-data-label">{ty?.phoneLabel || ''}</span>
                  <span className="ty-data-value">{orderData.phone}</span>
                </div>
              )}
              {orderData.address && (
                <div className="ty-data-row">
                  <span className="ty-data-label">{ty?.addressLabel || ''}</span>
