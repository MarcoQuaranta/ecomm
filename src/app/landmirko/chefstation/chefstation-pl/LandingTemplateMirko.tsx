            ...
            {slides.length > 0 && (
              <div className="gallery">
                <div className="gallery-main">
                  ...
                  <button className="gallery-arrow gallery-prev" onClick={prevSlide} aria-label={ariaLabels?.prevSlide || ''}>&#8249;</button>
                  <button className="gallery-arrow gallery-next" onClick={nextSlide} aria-label={ariaLabels?.nextSlide || ''}>&#8250;</button>
                </div>
                <div className="gallery-thumbs">
                  {visibleThumbs.map((slide: any, i: number) => {
                    const realIndex = thumbStart + i;
                    return (
                      <button key={realIndex} className={`gallery-thumb${currentSlide === realIndex ? ' active' : ''}`} onClick={() => { stopAutoplay(); setCurrentSlide(realIndex); }} aria-label={`${ariaLabels?.thumbnail || ''} ${realIndex + 1}`}>
            ...
            {features.items.map((f: any, i: number) => (
              <div key={i} className={`feature-block${i % 2 !== 0 ? ' feature-block-right' : ''}`}>
                ...
                <div className="feature-text">
                  {f.tag && <span className={`feature-tag tag-${f.tagColor || ''}`}>{f.tag}</span>}
                  <h3>{f.title}</h3>
        ...
        <section className="comparison-section">
          <div className="container">
            ...
            <table className="comparison-table">
              <thead>
                <tr>
                  <th>{comparison.featureColumn || ''}</th>
                  <th>{comparison.productColumn} <span className="winner-badge">{comparison.winnerBadge || ''}</span></th>
        ...
        <section className="reviews-section">
          <div className="container">
                ...
                <div key={index} className={`review-card${index < reviewsShown ? ' visible' : ''}`}>
                  <div className="review-top">
                    ...
                    <div className="review-meta">
                      <h4>{review.name}</h4>
                      {review.verified && (
                        <span className="review-badge">&#10003; {reviews?.verifiedBadge || ''}</span>
                      )}
                    </div>
                  </div>
                  ...
                  {review.sellerReply && (
                    <div className="seller-reply">
                      <span className="seller-reply-header"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: '4px' }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg> {reviews?.sellerReplyLabel || ''}</span>
            ...
            {reviewsShown < reviewItems.length && (
              <button className="show-more-btn" onClick={showMoreReviews}>{reviews.showMoreText || ''}</button>
            )}
      ...
      {reviewPopupData && (
        <div className={`review-popup-overlay${reviewPopupOpen ? ' open' : ''}`} onClick={() => setReviewPopupOpen(false)}>
          <div className="review-popup" onClick={(e) => e.stopPropagation()}>
            <button className="popup-close" onClick={() => setReviewPopupOpen(false)} aria-label={ariaLabels?.close || ''}>&times;</button>
            {!reviewSubmitted ? (
              <>
