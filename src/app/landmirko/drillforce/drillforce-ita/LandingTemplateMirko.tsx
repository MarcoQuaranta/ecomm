...
export default function LandingTemplateMirko({ content = {}, networkConfig }: { content?: any; networkConfig?: { endpoint: string; uid: string; key: string; offerId: string; lpId: string } }) {
  ...
  const kitContents = content?.kitContents;
    ...
    <div className="landing-mirko">
      ...
      {kitContents && (
        <section className="kit-contents-section">
          <div className="container">
            {kitContents.eyebrow && <span className="section-eyebrow">{kitContents.eyebrow}</span>}
            {kitContents.title && <h2 className="section-title">{kitContents.title}</h2>}
            {kitContents.image && (
              <div className="kit-contents-image">
                <Image src={kitContents.image} alt={kitContents.imageAlt || ''} width={800} height={500} style={{ width: '100%', height: 'auto', borderRadius: '12px' }} />
              </div>
            )}
            <div className="kit-contents-groups">
              {kitContents.groups.map((group: any, i: number) => (
                <div key={i} className="kit-group">
                  <h4 className="kit-group-title"><span className="kit-group-letter">{group.letter}</span> {group.title}</h4>
                  {group.items.length > 0 && (
                    <ul className="kit-group-items">
