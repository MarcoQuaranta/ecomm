...
export default function LandingTemplateMirko({ content = {}, networkConfig }: { content?: any; networkConfig?: { endpoint: string; uid: string; key: string; offerId: string; lpId: string } }) {
  ...
  const unboxing = content?.unboxing;
  ...
  const [unboxSlide, setUnboxSlide] = useState(0);
  const unboxSlides = unboxing?.slides || [];
  const totalUnboxSlides = unboxSlides.length;
  ...
  return (
    <div className="landing-mirko">
      ...
      {/* 7b. UNBOXING & GAMEPLAY */}
      {totalUnboxSlides > 0 && (
        <section className="unboxing-section">
          <div className="container">
            {unboxing.eyebrow && <span className="section-eyebrow">{unboxing.eyebrow}</span>}
            {unboxing.title && <h2 className="section-title">{unboxing.title}</h2>}
            <div className="unboxing-box">
              <div className="unboxing-carousel">
                <div className="unboxing-slide">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={unboxSlides[unboxSlide]?.src} alt={unboxSlides[unboxSlide]?.alt || ''} />
                </div>
                <button className="gallery-arrow gallery-prev" onClick={() => setUnboxSlide((p) => (p <= 0 ? totalUnboxSlides - 1 : p - 1))} aria-label={ariaLabels?.prevSlide || 'Precedente'}>&#8249;</button>
                <button className="gallery-arrow gallery-next" onClick={() => setUnboxSlide((p) => (p >= totalUnboxSlides - 1 ? 0 : p + 1))} aria-label={ariaLabels?.nextSlide || 'Successiva'}>&#8250;</button>
                <div className="unboxing-dots">
                  {unboxSlides.map((_: any, i: number) => (
                    <button key={i} className={`unboxing-dot${unboxSlide === i ? ' active' : ''}`} onClick={() => setUnboxSlide(i)} aria-label={`${i + 1}`} />
                  ))}
                </div>
              </div>
              {unboxing.caption && <p className="unboxing-caption">{unboxing.caption}</p>}
            </div>
          </div>
        </section>
      )}
