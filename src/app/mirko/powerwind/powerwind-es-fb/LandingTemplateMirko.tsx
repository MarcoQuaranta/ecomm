      ...
      {features?.items?.length > 0 && (
        <section className="features-section">
          <div className="container">
            ...
            {features.demoGifs?.items?.length > 0 && (() => {
              const [activeGif, setActiveGif] = React.useState(0);
              const gifs = features.demoGifs.items;
              return (
                <figure style={{ maxWidth: '500px', margin: '1.5rem auto 2.5rem', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.1)' }}>
                  <div style={{ position: 'relative' }}>
                    <img
                      src={gifs[activeGif].src}
                      alt={gifs[activeGif].alt || ''}
                      style={{ width: '100%', height: 'auto', display: 'block' }}
                    />
                    {gifs.length > 1 && (
                      <>
                        <button
                          onClick={() => setActiveGif((activeGif - 1 + gifs.length) % gifs.length)}
                          style={{ position: 'absolute', left: '8px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', color: '#fff', border: 'none', borderRadius: '50%', width: '36px', height: '36px', cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          aria-label="GIF precedente"
                        >&#8249;</button>
                        <button
                          onClick={() => setActiveGif((activeGif + 1) % gifs.length)}
                          style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', color: '#fff', border: 'none', borderRadius: '50%', width: '36px', height: '36px', cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          aria-label="GIF successiva"
                        >&#8250;</button>
                      </>
                    )}
                  </div>
                  <div style={{ background: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', padding: '0.5rem 1rem' }}>
                    {features.demoGifs.caption && (
                      <span style={{ color: '#fff', fontSize: '0.85rem', fontWeight: 500, letterSpacing: '0.03em' }}>{features.demoGifs.caption}</span>
                    )}
                    {gifs.length > 1 && (
                      <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem' }}>{activeGif + 1}/{gifs.length}</span>
                    )}
                  </div>
                </figure>
              );
